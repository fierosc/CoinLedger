...
- Entry fields: `name`, `level`, `reward`, `freq` (required); `egValue` (number or `null`; set it explicitly, never parse it from `reward`); `notes`; optional `abbr` (shown only in Dungeons/Instances), `link` / `link2`, `tasks` (`{task, kills, rewards[]}`), `breakdown` (`{task, title, rewards[]}`, with optional `breakdownLabel` to rename its "Task Breakdown" modal heading), and short display overrides `rewardShort`, `notesShort`, `locationShort` (the modal always shows the full text).
